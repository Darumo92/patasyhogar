...
// 1. Collect all unique inline EXECUTABLE script hashes
const hashes = new Set();
// Match <script> tags but capture the opening tag to check for type attribute
...
for (const file of walkHtml(DIST_DIR)) {
  ...
  while ((m = scriptRe.exec(html)) !== null) {
    ...
    // Skip non-executable types (JSON-LD, importmap, etc.)
...
// 2. Build the new script-src directive
...
// 4. Copy updated _headers to dist/
